...
from .aliengo_utils.random_footstep_generator import RandomFootstepGenerator
...
class AliengoRandom(BaseTask):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id,
        ...
        if self.is_footsteps:
            ...
            elif self.cfg['footstep_target_parameters']['gait'] == 'random':
                self.footstep_generator = RandomFootstepGenerator(self)
    ...
    def reset_last_action_buffers(self, env_ids):
        if self.cfg["actionSpace"] in {"pmtg_delta", "foot_positions_delta"}:
            ...
            self.last_action[env_ids] = self.reset_joint_angles
            self.last_last_action[env_ids] = self.reset_joint_angles
            self.last_last_last_action[env_ids] = self.reset_joint_angles
        elif self.cfg["actionSpace"] == "high_level":
            raise NotImplementedError()
        else:
            raise ValueError("Invalid action space")
