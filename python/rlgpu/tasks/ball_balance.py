...
class BallBalance(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        asset_root = "../../assets"
        asset_file = "mjcf/balance_bot.xml"
        ...
        for angle in self.leg_angles:
            # sensor local pose in the "tray" body of bbot
            sensor_pose = gymapi.Transform()
            sensor_pose.p.x = self.leg_outer_offset * math.cos(angle)
            sensor_pose.p.y = self.leg_outer_offset * math.sin(angle)  # changed
        ...
        for i in range(self.num_envs):
            ...
            bbot_tray_handle = self.gym.find_actor_rigid_body_handle(env_ptr, bbot_handle, "tray")

            env_sensors = []
            for sensor_pose in sensor_poses:
                sensor = self.gym.create_force_sensor(env_ptr, bbot_tray_handle, sensor_pose)
